...
void L2UIMap::Init()
{
	...
	// Сетка создается СТРОГО от 0 до максимума для стабильности массивов MyGUI
	for (int x = 0; x < MAP_TILES_X; x++)
	{
		...
		{
			// Рисуем сетку для ВСЕХ возможных квадратов, чтобы она не пропадала с экрана
			float tileWidth = 164;
	...
	if (btn == MyGUI::MouseButton::Left)
	{
		if (sender && sender->isType<MyGUI::ImageBox>() && sender != ui_map)
		{
			...
			for (uint16 x = 0; x < MAP_TILES_X; x++)
			{
				for (uint16 y = 0; y < MAP_TILES_Y; y++)
				{
					uint16 currentTileState = getTileState(x, y);
					if ((currentTileState & L2UIMTS_SELECTED) && !(currentTileState & L2UIMTS_ACTIVE))
					{
			...
			// ПРАВИЛЬНОЕ ИСПРАВЛЕНИЕ: Проверяем наличие файла отдельно, не ломая покраску сетки карты
			char sbuf[64];
...
void L2UIMap::setTileState(int x, int y, int16 state)
{
	...
	// Это предотвратит обращение к нулевому указателю и уберет крэш.
