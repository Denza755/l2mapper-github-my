...
void L2UIMap::Init()
{
	...
	for (int x = 0; x < MAP_TILES_X; x++)
	{
		...
		// ИСПРАВЛЕНИЕ: Выделяем память под массив состояний ВСЕГДА, до проверки файла
		...
		for (int y = 0; y < MAP_TILES_Y; y++)
		{
			...
			if (UPackageMgr.GetUPackage(buf))
			{
				...
				tbuf = new char[32]; // Безопасное выделение буфера под строку координаты
				sprintf(tbuf, "%d", x);
...
void L2UIMap::onMapTileLoadClick(MyGUI::Widget* sender)
{
	...
	for (uint16 x = 0; x < MAP_TILES_X; x++)
	{
		for (uint16 y = 0; y < MAP_TILES_Y; y++)
		{
			// ИСПРАВЛЕНИЕ: Добавлена проверка на выделение памяти, чтобы исключить чтение пустых ячеек
...
{
	// ЗАЩИТА: Если структура массивов не проинициализирована, возвращаем 0 вместо падения
	if (!tileStates || !tileStates[x])
		return 0;
